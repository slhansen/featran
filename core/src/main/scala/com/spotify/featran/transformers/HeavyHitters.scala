 ...
 * Only the top heavyHittersCount items are tracked, with 1.0 being the frequent rank, 2.0 the
  ...
  override val aggregator: Aggregator[String, SketchMap[String, Long], Map[String, (Int, Long)]] =
    SketchMap
      ...
      .andThenPresent { sm =>
        ...
        sm.heavyHitterKeys.foreach { k =>
          b += (k -> (rank, sketchMapParams.frequency(k, sm.valuesTable)))
          rank += 1
        }
        b.result()
      }
